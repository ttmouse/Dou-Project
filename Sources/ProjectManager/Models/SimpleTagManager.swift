...
class SimpleTagManagerImpl: ObservableObject {
    ...
    init(storage: TagStorage) {
        ...
        // 创建其他组件时使用弱引用避免循环
        self.projectOperations = ProjectOperationManager(delegate: nil, sortDelegate: nil, storage: storage)
        self.directoryWatcher = DirectoryWatcher(delegate: nil, operationManager: nil, storage: storage)
